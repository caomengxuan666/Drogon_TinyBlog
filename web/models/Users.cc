...
std::vector<Articles> Users::getArticle(const DbClientPtr &clientPtr) const {
    static const std::string sql = "select * from articles where author_id = ?";
    Result r(nullptr);
    {
    ...
    std::vector<Articles> ret;
    ret.reserve(r.size());
    for (auto const &row : r)
    {
        ret.emplace_back(Articles(row));
    }
    return ret;
}

void Users::getArticle(const DbClientPtr &clientPtr,
                       const std::function<void(std::vector<Articles>)> &rcb,

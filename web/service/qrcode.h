...
public:
    ...
    void generate_qr_code_with_filename(const std::string& save_path, const std::string& file_name, const std::string& data,
                                       int version, int box_size, int border) {
        // 初始化 Python 解释器
        py::scoped_interpreter guard{};
        ...
        try {
            ...
            // 使用指定的文件名
            std::string file_path = (py::str(os.attr("path").attr("join")(save_path, file_name))).cast<std::string>();
...
PYBIND11_MODULE(qrcode_cpp, m) {
    py::class_<QRCodeGenerator>(m, "QRCodeGenerator")
        ...
        .def("generate_qr_code_with_filename", &QRCodeGenerator::generate_qr_code_with_filename,
             py::arg("save_path"), py::arg("file_name"), py::arg("data"), py::arg("version"),
             py::arg("box_size"), py::arg("border"),
             "Generate a QR code using Python's qrcode library and save it as a PNG file with a specified filename.");
}